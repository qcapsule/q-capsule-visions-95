...
import { Download, FileText, Eye, Share2, Sparkles, CheckCircle, Star, Award, Users } from "lucide-react";
...
export const BrochureSection = forwardRef<HTMLElement>((props, ref) => {
  const brochureItems = [{
    ...
    description: "Detailed technical specifications, dimensions, and performance metrics for all Q Capsule models.",
    stat: "100%",
    statLabel: "Detailed"
  }, {
    icon: Star,
    title: "Design Gallery",
    description: "High-quality images showcasing different configurations, interior options, and customization possibilities.",
    stat: "50+",
    statLabel: "Designs"
  }, {
    icon: Award,
    title: "Customization Guide",
    description: "Comprehensive guide to personalizing your Q Capsule experience with premium materials and finishes.",
    stat: "24/7",
    statLabel: "Support"
  }];
  ...
  return <section ref={ref} id="brochure" className="h-[100vh] flex items-center justify-center overflow-hidden sticky top-0" style={{
    ...
    backgroundRepeat: "no-repeat"
      ...
      {/* Background for GSAP */}
      <div className="brochure-bg absolute inset-0 bg-cover bg-center bg-no-repeat" style={{
      backgroundImage: `url(${constructionCapsuleImage})`
    }}></div>
      ...
      <div className="brochure-content container mx-auto px-6 relative z-10 flex items-center justify-center h-full">
        <div className="text-center max-w-6xl">
          <h2 className="text-5xl lg:text-7xl font-bold leading-tight text-white drop-shadow-lg mb-6">
            Explore Our{" "}
            <span className="text-white drop-shadow-lg">Brochure
          </span>
          </h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8 max-w-4xl mx-auto">
            {brochureItems.map((item, index) => <div key={item.title} className="parallax-element bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 hover:bg-white/20 hover:border-white/40 hover:scale-105 transition-all duration-300 cursor-pointer group">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <button onClick={handleDownload} className="bg-white/20 backdrop-blur-md border border-white/30 text-white hover:bg-white/30 transition-all duration-300 rounded-full px-8 py-4 text-lg font-medium flex items-center justify-center gap-3">
              <Download className="h-5 w-5" />
              Download Brochure
            </button>
            <button onClick={handleView} className="bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-white/20 transition-all duration-300 rounded-full px-8 py-4 text-lg font-medium flex items-center justify-center gap-3">
